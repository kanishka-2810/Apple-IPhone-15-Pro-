...
const VideoCarousel = () => {
    ...
    useGSAP(() => {
        ...
        // to play the video when it is in view
        gsap.to("#video", {
            scrollTrigger: {
                trigger: "#video",
                toggleActions: "restart none none none" //action when it first comes to view (none) are the other sides from where it comes to view
            },
            onComplete: () => {
                setVideo((prev) => ({
                    ...prev,
        ...
        if (span[videoId]) {
            ...
            let anim = gsap.to(span[videoId], {
                ...
                onComplete: () => {
                    if (isPlaying) {
                        gsap.to(videoDivRef.current[videoId], {
                            width: '12px' // bring back to original size
                        })
                        gsap.to(span[videoId], {
                            backgroundColor: "#afafaf" //bringing back to original color 'gray'
                        })
                    }
                },
            })
                ...
                <div className='flex-center py-5 px-7 bg-gray-300 backdrop-blur rounded-full'>
                    {videoRef.current.map((_, i) => (
                        <span key={i}
                            ...
                            className='mx-2 w-3 h-3 bg-gray-200 rounded-full relative pointer-cursor'
                        >
